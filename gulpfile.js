...
var morgans = {
    ...
    src: {
        html: 'src/*.html',
        js: 'src/js/morgans.js',
        style: 'src/scss/morgans.scss',
        img: 'src/img/**/*.*',
        fonts: 'src/fonts/**/*.*'
    },
    watch: {
...
gulp.task('html:build', function () {
    gulp.src(morgans.src.html)
        .pipe(plugins.rigger())
        .pipe(gulp.dest(morgans.build.html))
        .pipe(reload({stream: true}));
});
...
gulp.task('watch', function(){
    plugins.watch([morgans.watch.html], function(event, cb) {
        gulp.start('html:build');
    });
    plugins.watch([morgans.watch.style], function(event, cb) {
        gulp.start('style:build');
    });
    plugins.watch([morgans.watch.js], function(event, cb) {
        gulp.start('js:build');
    });
    plugins.watch([morgans.watch.img], function(event, cb) {
        gulp.start('image:build');
    });
    plugins.watch([morgans.watch.fonts], function(event, cb) {
